...
export async function getStory(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const data = _getPageStoryLocal(parseInt(pageId));
    const response: Object = {
      meta: {
        count: 1,
        title: 'story',
...
export async function getChoices(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { bookId, pageId } = req.params;
    const data = _getPageOptionsLocal(parseInt(pageId));
    const response: Object = {
      meta: {
        count: 1,
        title: 'choices',
 ...
 * @returns {string}
 */
function _getPageStoryLocal(page){
  switch (page) {
 ...
 * @param {number | null | undefined} page
 */
function _getPageOptionsLocal(page){
  switch (page) {
    case 1: return [
